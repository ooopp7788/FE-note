...
channel.port1.onmessage = function(event) {
    ...
    // frameDeadline 估算的下一帧执行时刻, 简单判断超时, 不一定准确
    if (frameDeadline - currentTime <= 0) {
     // prevTimeoutTime: 表示精准的超时时刻, 通过 requestHostCallback 传入
     // 类似 requestIdelCallback 的超时配置
...
> flushWork
1. 根据入参 `didTimeout` 简单判断是否超时, 如果初步判断超时, 遍历 `firstCallbackNode`, 根据 `firstCallbackNode.expirationTime < currentTime` 判断准确的超时状态, 如果确实超时, 执行 `flushFirstCallback`
2. 如果 `didTimeout` 为假, 遍历 `firstCallbackNode`, 根据`shouldYieldToHost()` 判断当前帧是否还有剩余时间, 有剩余时间才执行 `flushFirstCallback`
```TS
...
#### 小结
1. `requestHostCallBack(flushWork)`: 用于注册帧回调, 注册一次, 回调执行一次
2. `firstCallbackNode`: callback 任务链表(环形双向链表), 所有任务都放在链表中等待执行
3. `flushWork`: 控制 callback 执行, `requestHostCallBack` 注册的就是 `flushWork`, `flushWork` 用于规划一帧的时间执行多少个 callback
4. `flushImmediateWork`: 遍历链表执行优先级为 `ImmediatePriority` 的节点回调, 
5. 执行过程:
    1. `requestHostCallBack(flushWork)` 启动当前帧任务, 初步估算下一帧时间, 当前帧是否超时, 并在多帧间做帧时间修正
    2. `flushWork` 参考入参 `didTimeout`, 以 `firstCallbackNode.expirationTime < currentTime` 的比较结果判断 callback 是否超时, 如果超时调用 `flushFirstCallback`, 执行 callback
    3. `flushFirstCallback`, 执行 callback 后, callback() 返回值可能也是一个 callback, 将(按超时时间序)其插入 `firstCallbackNode` 链表
    ...
    5. 当前帧结束前, 调用 `flushImmediateWork`, 遍历链表, 如果优先级为 `ImmediatePriority` 则执行节点回调, 并继续遍历, 否则 break
    6. 再次判断链表剩余节点, 执行 `ensureHostCallbackIsScheduled`, 任务进入下一帧, goto(step1)
